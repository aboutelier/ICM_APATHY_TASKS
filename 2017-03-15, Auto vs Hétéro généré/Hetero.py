...
##########################################DEBUT HETERO-GENERE##############################################################################################################################
	...
	def attente(self,event):
		self.hasardbleu1()
		self.startchrono()
		self.after(263000, self.time)
		self.after(263000, self.timeauto)
	...
	def sec(self):
		if self.tempssecondes==1 and self.tempsminutes==2:
			...
			self.after(20000, self.prepaauto)
	...
	def hasardbleu4(self):
		if self.nbblue3==self.coord0:
			if self.tourverif<1:
				self.coordpossibles=[self.coord1, self.coord3, self.coord4]
			self.nbblue4=random.choice(self.coordpossibles)
			self.verifbleu4()
